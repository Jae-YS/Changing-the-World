...
import "../Styles/ChatBot.css"; // Make sure you have this CSS file
...
const ChatApp = () => {
  ...
  const updateAiChat = (id, text) => {
    setChats((currentChats) =>
      currentChats.map((chat) => (chat.id === id ? { ...chat, text } : chat))
    );
  };
  ...
  return (
    <div id="app">
          ...
          <div key={chat.id} className={`wrapper ${chat.isAi ? "ai" : ""}`}>
            <div className="chat">
              <div className="profile">
                <img
                  src={chat.isAi ? bot : user}
                  alt={chat.isAi ? "bot" : "user"}
                />
              </div>
      ...
      <form className="formInput" onSubmit={handleSubmit}>
        ...
        />
        <button id="button-chat" type="submit">
